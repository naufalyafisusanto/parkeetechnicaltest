...
rsync -avz "$source_directory" "$username@$server_ip:~/"

if [ $? -eq 0 ]; then
  echo "Directory $source_directory copied to $username@$server_ip successfully using Rsync."
else
  echo "Failed to copy directory $source_directory to $username@$server_ip using Rsync."
fi